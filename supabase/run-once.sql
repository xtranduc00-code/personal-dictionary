-- Run once in Supabase SQL Editor: drop then create (words + IELTS).
...
create index words_updated_at_idx on public.words(updated_at desc);
create index words_is_saved_idx on public.words(is_saved) where is_saved = true;

alter table public.words enable row level security;
drop policy if exists "Allow anon all on words" on public.words;
create policy "Allow anon all on words" on public.words for all to anon using (true) with check (true);
