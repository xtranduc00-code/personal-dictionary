...
import { authFetch, getAuthToken } from "./auth-context";
const WORDS_API = "/api/words";
function useWordsAPI(): boolean {
    return hasSupabaseEnv && Boolean(getAuthToken());
}
function parseSense(s: unknown): DictionarySense | null {
    ...
    return {
        ...
        collocations: Array.isArray(o.collocations) ? o.collocations.map(String) : [],
        phrasalVerbs: Array.isArray(o.phrasalVerbs) ? o.phrasalVerbs.map(String) : [],
...
function sortByUpdatedAtDesc(words: WordRow[]) {
    return [...words].sort((a, b) => new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime());
}
function normalizeWordRow(input: Partial<WordRow>): WordRow | null {
    ...
    const senses: DictionarySense[] = Array.isArray(input.senses) && input.senses.length > 0
...
function saveToLocalStorage(words: WordRow[]) {
    if (typeof window === "undefined")
        return;
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify(sortByUpdatedAtDesc(words)));
}
function rowFromDb(r: Record<string, unknown>): WordRow {
    const rawSenses = r.senses;
    const senses: DictionarySense[] = Array.isArray(rawSenses) && rawSenses.length > 0
...
async function loadFromWordsAPI(): Promise<WordRow[]> {
    const res = await authFetch(WORDS_API);
    if (!res.ok) {
        markSupabaseUnavailable();
        return applyHistoryLimit(loadFromLocalStorage());
    }
    ...
    return applyHistoryLimit(rows.map((r: Record<string, unknown>) => rowFromDb(r)));
}
export async function loadWordsFromStorage(): Promise<WordRow[]> {
    if (useWordsAPI())
        return loadFromWordsAPI();
    if (useSupabase())
    ...
    if (useWordsAPI()) {
        try {
            const res = await fetch(WORDS_API, {
                ...
                body: JSON.stringify({ word: entry.word, senses }),
            });
            if (!res.ok) {
                markSupabaseUnavailable();
...
function upsertLocal(entry: DictionaryEntry, normalizedWord: string, now: string): WordRow {
...
export async function deleteWordFromStorage(id: string): Promise<void> {
    if (useWordsAPI()) {
        const res = await authFetch(`${WORDS_API}/${id}`, { method: "DELETE" });
        if (!res.ok) {
            markSupabaseUnavailable();
            const all = loadFromLocalStorage();
...
export async function setSavedStatusById(id: string, isSaved: boolean): Promise<WordRow | null> {
    if (useWordsAPI()) {
        const res = await authFetch(`${WORDS_API}/${id}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
...
export async function setSavedStatusByWord(word: string, isSaved: boolean): Promise<WordRow | null> {
...
export async function getWordFromStorage(word: string): Promise<WordRow | null> {
    ...
    if (!normalizedWord)
        return null;
    if (useWordsAPI()) {
        const res = await authFetch(`${WORDS_API}?word=${encodeURIComponent(normalizedWord)}`, {});
        if (!res.ok) {
            markSupabaseUnavailable();
