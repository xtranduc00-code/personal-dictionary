...
/** Same as SpeakingQuestion with topicName (e.g. for exam view). */
export type SpeakingQuestionWithTopic = SpeakingQuestion & { topicName: string };

export type VocabItem = { word: string; explanation: string };
...
export async function getTopics(): Promise<Topic[]> {
  const data = await getJson<{ id: string; name: string; created_at: string }[]>(`${API}/topics`);
  return data.map((r) => ({ id: r.id, name: r.name, createdAt: r.created_at })).sort(
    (a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime(),
  );
}

export async function addTopic(name: string): Promise<Topic> {
  const r = await postJson<{ id: string; name: string; created_at: string }>(`${API}/topics`, { name });
  return { id: r.id, name: r.name, createdAt: r.created_at };
}

export async function getTopic(id: string): Promise<Topic | null> {
  try {
    const r = await getJson<{ id: string; name: string; created_at: string }>(`${API}/topics/${id}`);
    return { id: r.id, name: r.name, createdAt: r.created_at };
  } catch {
    return null;
  }
...
export async function getQuestions(topicId: string): Promise<SpeakingQuestion[]> {
  const data = await getJson<{ id: string; topic_id: string; text: string; part: string; created_at: string }[]>(
...
export async function getQuestionsByPart(
  topicId: string,
  part: SpeakingPart,
...
/** All questions from all topics, grouped by part (for exam view). */
export async function getAllQuestionsByPart(): Promise<{
  ...
  const data = await getJson<{
    part1: { id: string; topicId: string; topicName: string; text: string; part: string; createdAt: string }[];
    part2: { id: string; topicId: string; topicName: string; text: string; part: string; createdAt: string }[];
    part3: { id: string; topicId: string; topicName: string; text: string; part: string; createdAt: string }[];
  }>(`${API}/exam-questions`);
  const map = (r: { id: string; topicId: string; topicName: string; text: string; part: string; createdAt: string }) => ({
    id: r.id,
    topicId: r.topicId,
...
export async function addQuestion(
  ...
  const data = await postJson<{ count: number }>(`${API}/topics/${topicId}/questions`, { part, text: trimmed });
  const list = await getJson<{ id: string; topic_id: string; text: string; part: string; created_at: string }[]>(
    `${API}/topics/${topicId}/questions`,
  );
  const added = list.find((q) => q.text === trimmed && q.part === part);
  if (!added) throw new Error("Failed to get added question");
  return {
    id: added.id,
...
export async function addQuestions(
  ...
  const data = await postJson<{ count: number }>(`${API}/topics/${topicId}/questions`, {
    part,
    texts: lines,
  });
  return data.count ?? 0;
...
export async function updateQuestion(
  id: string,
  updates: { text?: string; part?: SpeakingPart },
): Promise<void> {
...
export async function addTopicVocabItem(
  topicId: string,
  word: string,
  explanation: string,
): Promise<void> {
  await postJson(`${API}/topics/${topicId}/vocab`, { word: word.trim(), explanation: explanation.trim() });
}
...
export async function saveDraft(questionId: string, transcript: string): Promise<void> {
  await postJson(`${API}/questions/${questionId}/practice`, { draft: transcript });
}
