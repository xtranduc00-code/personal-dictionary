...
import { getDriveModalPortalContainer } from "@gd/lib/portal-container";
import { buttonVariants } from "@gd/components/ui/button";
const AlertDialog = AlertDialogPrimitive.Root;
const AlertDialogTrigger = AlertDialogPrimitive.Trigger;
const AlertDialogPortal = AlertDialogPrimitive.Portal;
const AlertDialogOverlay = React.forwardRef<React.ElementRef<typeof AlertDialogPrimitive.Overlay>, React.ComponentPropsWithoutRef<typeof AlertDialogPrimitive.Overlay>>(({ className, ...props }, ref) => (<AlertDialogPrimitive.Overlay className={cn("pointer-events-auto fixed inset-0 z-[99] min-h-[100dvh] w-screen bg-black/80 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0", className)} {...props} ref={ref}/>));
AlertDialogOverlay.displayName = AlertDialogPrimitive.Overlay.displayName;
const AlertDialogContent = React.forwardRef<React.ElementRef<typeof AlertDialogPrimitive.Content>, React.ComponentPropsWithoutRef<typeof AlertDialogPrimitive.Content>>(({ className, children, ...props }, ref) => (<AlertDialogPortal container={getDriveModalPortalContainer()}>
    <AlertDialogOverlay />
    <AlertDialogPrimitive.Content ref={ref} className={cn("group pointer-events-none fixed inset-0 z-[100] flex items-center justify-center overflow-y-auto border-0 bg-transparent p-4 shadow-none outline-none", "data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0")} {...props}>
      <div className={cn("pointer-events-auto grid w-full max-w-lg gap-4 border bg-background p-6 shadow-lg duration-200 sm:rounded-lg", "group-data-[state=closed]:zoom-out-95 group-data-[state=open]:zoom-in-95", "max-h-[min(90dvh,900px)] overflow-y-auto", className)}>
        {children}
      </div>
