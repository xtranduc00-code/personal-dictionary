...
function asText(buf: Buffer): string {
    // Assume utf-8; if the user needs encoding detection later, we can add it.
...
export async function POST(req: Request) {
    ...
    try {
        ...
        const nameWithExt = baseName.toLowerCase().endsWith(ext) ? baseName : `${baseName}${ext}`;
        ...
        console.error("opensubtitles import", e);
