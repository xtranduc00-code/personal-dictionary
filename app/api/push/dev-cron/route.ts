...
/** Dev-only: runs the full reminder sweep without CRON_SECRET. */
export async function POST() {
  ...
  const [calendar, studySchedule] = await Promise.all([
    runCalendarReminderSweep(db, siteUrl),
    runStudyScheduleReminderSweep(db, siteUrl),
  ]);
  return NextResponse.json({ ok: true, calendar, studySchedule });
}
