...
import { ChevronLeft, ChevronRight, Headphones, LayoutGrid, X, Maximize2, Minimize2 } from "lucide-react";
import { getListeningAudioUrls, getListeningTest } from "@/lib/listening-data";
import { getPartContent, getChooseTwoBlocksForPart } from "@/lib/listening-part-content";
import { normalizeListeningAnswer, rawScoreToBand, saveListeningResultToHistory, } from "@/lib/listening-utils";
import { TOTAL_PARTS } from "./constants";
import { getSelectionHighlightInfo, useFullscreen } from "@/components/ielts";
import { AudioPlayer, HighlightToolbar, HighlightsContext, Part1Content, TranscriptWithHighlights, type Highlight, } from "./components";
const Part2Content = dynamic(() => import("./components/Part2Content").then((m) => ({ default: m.Part2Content })), { ssr: false });
const Part3Content = dynamic(() => import("./components/Part3Content").then((m) => ({ default: m.Part3Content })), { ssr: false });
const Part4Content = dynamic(() => import("./components/Part4Content").then((m) => ({ default: m.Part4Content })), { ssr: false });
...
const AddFlashcardModal = dynamic(() => import("./components").then((m) => m.AddFlashcardModal), { ssr: false });
const ResultModal = dynamic(() => import("./components").then((m) => m.ResultModal), { ssr: false });
export default function ListeningTestPage() {
    ...
    const data = useMemo(() => getListeningTest(params.setId, params.testId), [params.setId, params.testId]);
    const set = data?.set;
    ...
    const [pendingScrollToQuestion, setPendingScrollToQuestion] = useState<number | null>(null);
    const contentRef = useRef<HTMLDivElement>(null);
    const [resolvedAudioUrl, setResolvedAudioUrl] = useState<string | null>(null);
    const [audioProbeFailed, setAudioProbeFailed] = useState(false);
    const [audioFallbackIndex, setAudioFallbackIndex] = useState(0);
    ...
    const transcriptHtml = useMemo(() => (set && data ? getListeningTranscriptHtml(set.id, data.id, currentPart) : undefined), [set, data, currentPart]);
    ...
    useEffect(() => {
        const onMouseUp = () => {
            ...
            const segmentEl = contentRef.current?.querySelector<HTMLElement>(`[data-segment-id="${info.segmentId}"]`) ??
                transcriptRef.current?.querySelector<HTMLElement>(`[data-segment-id="${info.segmentId}"]`);
    ...
    const audioUrls = useMemo(() => (data && set ? getListeningAudioUrls(set, data, currentPart) : []), [data, set, currentPart]);
    ...
    useEffect(() => {
        ...
        if (isCrossOrigin(firstUrl)) {
            setResolvedAudioUrl(firstUrl);
        ...
        let cancelled = false;
        (async () => {
            const check = (url: string) => fetch(url, { method: "HEAD" })
                .then((r) => (r.ok ? url : null))
                .catch(() => null);
            let pending = audioUrls.length;
            let done = false;
            const tryResolve = (url: string | null) => {
                if (done || cancelled)
                    return;
                if (url !== null) {
                    done = true;
            ...
            audioUrls.forEach((url) => check(url).then(tryResolve));
        })();
        return () => {
            cancelled = true;
        };
    }, [audioUrls]);
    const computeCorrectCount = useCallback((ans: Record<number, string>) => {
        if (!correctAnswers)
            return 0;
        return Object.keys(correctAnswers)
            ...
            if (Array.isArray(expected)) {
                return expected.some((item) => normalizeListeningAnswer(String(item)) === userAnswer);
            }
    ...
    const getChooseTwoSelection = useCallback((primaryQ: number, secondaryQ?: number) => {
    ...
    const toggleChooseTwo = useCallback((primaryQ: number, secondaryQ: number, letter: string) => {
    ...
    const resultText = correctAnswers
        ...
        : t("submittedNoKey");
    const getCorrectAnswerText = useCallback((qNum: number): string | null => {
        if (!correctAnswers)
            return null;
        const expected = correctAnswers[qNum];
    ...
    const scrollToAnswer = useCallback((qNum: number) => {
        if (!transcriptRef.current)
            return;
        const el = transcriptRef.current.querySelector<HTMLElement>(`[id^="ielts-listening-explanation-number-${qNum}-"]`);
        el?.scrollIntoView({ behavior: "smooth", block: "center" });
    }, []);
    const scrollToQuestion = useCallback((qNum: number) => {
        const container = questionsPanelRef.current ?? contentRef.current;
        const el = container?.querySelector<HTMLElement>(`[data-question-number="${qNum}"]`);
        el?.scrollIntoView({ behavior: "smooth", block: "center" });
    }, []);
    useEffect(() => {
        if (pendingScrollToQuestion == null)
            return;
        const raf = requestAnimationFrame(() => {
            const container = questionsPanelRef.current ?? contentRef.current;
            const el = container?.querySelector<HTMLElement>(`[data-question-number="${pendingScrollToQuestion}"]`);
            el?.scrollIntoView({ behavior: "smooth", block: "center" });
    ...
    if (!data)
        return notFound();
    const partContent = useMemo(() => getPartContent(data.set.id, data.id), [data.set.id, data.id]);
    const partQuestionButtons = useMemo(() => {
        const start = (currentPart - 1) * 10 + 1;
        const end = currentPart * 10;
        const chooseTwoBlocks = getChooseTwoBlocksForPart(partContent, currentPart as 2 | 3);
        const pairFirstSet = new Set(chooseTwoBlocks?.map((b) => b.qNums[0]) ?? []);
        const pairSecondSet = new Set(chooseTwoBlocks?.flatMap((b) => (b.qNums.length >= 2 ? [b.qNums[1]] : [])) ?? []);
        const items: Array<{
            type: "single";
            qNum: number;
        } | {
        ...
        for (let qNum = start; qNum <= end; qNum++) {
            if (pairFirstSet.has(qNum)) {
                ...
                if (block && block.qNums.length >= 2) {
                    items.push({ type: "pair", qNums: [block.qNums[0], block.qNums[1]] as [
                            number,
                            number
                        ] });
                }
    ...
    const currentPartJsx = useMemo(() => {
        ...
        const partProps = { answers, updateAnswer, isCorrect, submitted, segmentIdPrefix, getCorrectAnswerText };
        switch (currentPart) {
            case 1:
                return <Part1Content {...partProps} content={partContent.part1}/>;
            case 2:
                return (<Part2Content {...partProps} getChooseTwoSelection={getChooseTwoSelection} toggleChooseTwo={toggleChooseTwo} content={partContent.part2}/>);
            case 3:
                return (<Part3Content {...partProps} getChooseTwoSelection={getChooseTwoSelection} toggleChooseTwo={toggleChooseTwo} content={partContent.part3}/>);
            case 4:
    ...
    return (<div className="ielts-exam mx-auto max-w-6xl space-y-4">
      <nav className="flex items-center justify-between rounded-b-lg border-b border-zinc-200 bg-zinc-100 px-5 py-3 text-sm text-zinc-700 shadow-sm dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-200">
        ...
        <div className="flex items-center gap-2">
          ...
          <Tooltip content={t("ariaBackToTestList")}>
            <Link href="/listening" className="inline-flex cursor-pointer items-center gap-1.5 rounded-lg bg-zinc-200 px-3 py-2 text-sm font-medium text-zinc-800 hover:bg-zinc-300 dark:bg-zinc-700 dark:text-zinc-100 dark:hover:bg-zinc-600" aria-label={t("ariaBackToTestList")}>
      ...
      <section className="rounded-2xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              <Tooltip content={t("ariaPreviousPart")}>
                <button type="button" aria-label={t("ariaPreviousPart")} disabled={currentPart === 1} onClick={() => setCurrentPart((p) => Math.max(1, p - 1))} className="flex h-9 w-9 cursor-pointer items-center justify-center rounded border border-zinc-300 bg-white text-zinc-600 transition hover:bg-zinc-100 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:bg-white dark:border-zinc-600 dark:bg-zinc-900 dark:text-zinc-300 dark:hover:bg-zinc-800 dark:disabled:hover:bg-zinc-900">
                  <ChevronLeft className="h-4 w-4"/>
                </button>
              </Tooltip>
              ...
              <span className="font-semibold">{t(PART_LABEL_KEYS[currentPart])}</span>
              <Tooltip content={t("ariaNextPart")}>
                <button type="button" aria-label={t("ariaNextPart")} disabled={currentPart === TOTAL_PARTS} onClick={() => setCurrentPart((p) => Math.min(TOTAL_PARTS, p + 1))} className="flex h-9 w-9 cursor-pointer items-center justify-center rounded border border-zinc-300 bg-white text-zinc-600 transition hover:bg-zinc-100 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:bg-white dark:border-zinc-600 dark:bg-zinc-900 dark:text-zinc-300 dark:hover:bg-zinc-800 dark:disabled:hover:bg-zinc-900">
                  <ChevronRight className="h-4 w-4"/>
                </button>
              </Tooltip>
            </div>
          </div>
          {audioUrls.length > 0 ? (resolvedAudioUrl ? (<AudioPlayer key={`${currentPart}-${resolvedAudioUrl}`} src={resolvedAudioUrl}/>) : audioProbeFailed ? (audioFallbackIndex < audioUrls.length ? (<AudioPlayer key={`${currentPart}-fallback-${audioFallbackIndex}`} src={audioUrls[audioFallbackIndex]} onError={() => setAudioFallbackIndex((i) => i + 1)}/>) : (<p className="text-zinc-500 dark:text-zinc-400">
                  {t("noAudio")}
                </p>)) : (<p className="text-zinc-500 dark:text-zinc-400">
                {t("loadingAudio")}
              </p>)) : (<p className="text-zinc-500 dark:text-zinc-400">
              {t("noAudio")}
            </p>)}
        ...
        <div ref={contentRef} className="relative border-t border-zinc-100 px-6 py-6 text-base text-zinc-800 dark:border-zinc-800 dark:text-zinc-100">
          <HighlightsContext.Provider value={highlightsValue}>
            <ScrollToAnswerContext.Provider value={submitted && transcriptHtml ? scrollToAnswer : null}>
            {submitted && transcriptHtml ? (<div ref={splitContainerRef} className="flex h-[70vh] overflow-hidden">
                
                <div className="flex flex-col overflow-hidden" style={{ width: `${transcriptPct}%` }}>
                ...
                const qNum = item.type === "single" ? item.qNum : item.qNums[0];
                const correct = item.type === "single" ? isCorrect(item.qNum) : isCorrect(item.qNums[0]);
                const label = item.type === "single" ? String(item.qNum) : `${item.qNums[0]}–${item.qNums[1]}`;
                const base = "flex h-7 min-w-7 items-center justify-center rounded-md border px-1 text-xs font-semibold cursor-pointer transition-colors hover:opacity-90";
                ...
                return (<Tooltip key={label} content={t("scrollToQuestionInTranscriptTooltip").replace("{n}", label)}>
                  ...
                  <TranscriptWithHighlights ref={transcriptRef} transcriptHtml={transcriptHtml} segmentId={`transcript-${currentPart}`} highlights={highlights} removeHighlight={removeHighlight} answers={answers} correctAnswers={correctAnswers} isCorrect={isCorrect} getCorrectAnswerText={getCorrectAnswerText} getBadgeLabel={transcriptBadgeHelpers.getBadgeLabel} getBadgeCorrect={transcriptBadgeHelpers.getBadgeCorrect}/>
                </div>

                
                <div onMouseDown={(e) => { isDraggingRef.current = true; e.preventDefault(); }} className="group relative mx-1 flex shrink-0 cursor-col-resize flex-col items-center justify-center" style={{ width: 12 }}>
                  ...
                  <div className="absolute flex h-8 w-4 items-center justify-center rounded border border-zinc-300 bg-white shadow-sm group-hover:bg-zinc-50 dark:border-zinc-600 dark:bg-zinc-800 dark:group-hover:bg-zinc-700">
                    <svg viewBox="0 0 8 16" fill="currentColor" className="h-4 w-2 text-zinc-400">
                ...
                <div ref={questionsPanelRef} className="flex-1 overflow-y-auto pl-3">
              ...
              </div>) : (<div ref={questionsPanelRef} className="space-y-4">
                <div className="mb-5">
                  <p className="text-2xl font-bold text-zinc-900 dark:text-zinc-100">
                    SECTION {currentPart}
                  </p>
            ...
            {toolbar?.show && (<HighlightToolbar x={toolbar.x} y={toolbar.y} hasHighlightId={!!toolbar.highlightId} selectedText={toolbar.selectedText} flashcardText={toolbar.selectedTextWords ?? toolbar.selectedText} onHighlight={handleHighlightClick} onUnhighlight={handleUnhighlightClick} onFlashcard={handleFlashcardClick} showHighlightButtons={!submitted}/>)}
        ...
        <footer className="flex flex-col gap-3 border-t border-zinc-200 px-6 py-5 dark:border-zinc-800">
          {!submitted ? (<>
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {partQuestionButtons.map((item) => {
                const qNum = item.type === "single" ? item.qNum : item.qNums[0];
                const answered = item.type === "single"
                    ? (answers[item.qNum] ?? "").trim() !== ""
                    : item.qNums.some((n) => (answers[n] ?? "").trim() !== "");
                const label = item.type === "single" ? String(item.qNum) : `${item.qNums[0]}–${item.qNums[1]}`;
                return (<Tooltip key={label} content={t("scrollToQuestionTooltip").replace("{n}", label)}>
                      <button type="button" onClick={() => scrollToQuestion(qNum)} className={`inline-flex h-9 min-w-9 cursor-pointer items-center justify-center rounded-lg border px-1.5 text-sm font-medium transition-colors hover:opacity-90 ${answered
                        ? "border-emerald-500 bg-emerald-50 text-emerald-800 dark:border-emerald-600 dark:bg-emerald-950/50 dark:text-emerald-200"
                        : "border-zinc-200 bg-zinc-50 text-zinc-600 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-400"}`} aria-label={`Question ${label}`}>
                        {label}
                      </button>
                    </Tooltip>);
            })}
              ...
              <div className="flex flex-wrap items-center justify-center gap-3">
                ...
                const done = Array.from({ length: 10 }, (_, i) => start + i).filter((q) => (answers[q] ?? "").trim() !== "").length;
                const isCurrent = currentPart === section;
                return (<Tooltip key={section} content={t("goToSectionTooltip").replace("{n}", String(section))}>
                      <button type="button" onClick={() => setCurrentPart(section)} className={`flex cursor-pointer items-center gap-1.5 rounded-lg border px-3 py-2 text-sm transition-colors hover:opacity-90 ${isCurrent
                        ? "border-red-500 bg-red-50 text-red-800 dark:border-red-600 dark:bg-red-950/50 dark:text-red-200"
                        : "border-zinc-200 bg-zinc-50 text-zinc-600 hover:bg-zinc-100 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700"}`} aria-label={t("sectionLabel").replace("{n}", String(section))} aria-current={isCurrent ? "true" : undefined}>
                        <span className="font-medium">{t("sectionLabel").replace("{n}", String(section))}</span>
                        <span className="text-zinc-500 dark:text-zinc-400">|</span>
                        <span>{t("doneCount").replace("{done}", String(done)).replace("{total}", "10")}</span>
                      </button>
                    </Tooltip>);
            })}
      ...
      {showQuestionBoard && (<div className="fixed inset-0 z-40 flex cursor-pointer items-end justify-center bg-black/30 sm:items-center" onClick={() => setShowQuestionBoard(false)} role="dialog" aria-modal="true" aria-labelledby="question-board-title">
      ...
      {showSubmitConfirm && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true" aria-labelledby="submit-confirm-title">
          <div className="w-full max-w-sm rounded-xl border border-zinc-200 bg-white p-5 shadow-xl dark:border-zinc-700 dark:bg-zinc-900">
            <h2 id="submit-confirm-title" className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">
      ...
      {showResultModal && (<ResultModal onClose={closeResultModal} correctAnswers={correctAnswers} answers={answers} isCorrect={isCorrect}/>)}
