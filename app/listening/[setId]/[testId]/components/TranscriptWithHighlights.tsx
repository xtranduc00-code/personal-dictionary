...
function applyAnswerFeedbackToTranscript(container: HTMLElement, isCorrect: (qNum: number) => boolean | null, getBadgeLabel?: (qNum: number) => string, getBadgeCorrect?: (qNum: number) => boolean | null): void {
    container.querySelectorAll(`p.${TRANSCRIPT_SENTENCE_WRONG}`).forEach((p) => {
        p.classList.remove(TRANSCRIPT_SENTENCE_WRONG);
    });
    const badges = container.querySelectorAll<HTMLElement>("[id^='ielts-listening-explanation-number-']");
    badges.forEach((badge) => {
        ...
        const mark = el as HTMLElement | null;
        mark?.classList.remove("transcript-answer-wrong", "explanation-highlight");
        badge.classList.remove("transcript-badge-wrong", "transcript-badge-correct");
        if (correct === true) {
            mark?.classList.add("explanation-highlight");
            badge.classList.add("transcript-badge-correct");
        }
        else if (correct === false) {
            mark?.classList.add("transcript-answer-wrong");
            badge.classList.add("transcript-badge-wrong");
            const paragraph = badge.closest("p");
            if (paragraph)
                paragraph.classList.add(TRANSCRIPT_SENTENCE_WRONG);
        }
    });
}
...
function createRangeForOffset(container: HTMLElement, start: number, end: number): Range | null {
...
function stripExplanationHighlight(html: string): string {
    return html
        .replace(/\bexplanation-highlight\b/g, "")
        .replace(/class="([^"]*)"/g, (_, c) => {
        const trimmed = c.replace(/\s+/g, " ").trim();
        return trimmed ? `class="${trimmed}"` : 'class=""';
    });
}
function applyHighlightsToTranscript(container: HTMLElement, originalHtml: string, segmentHighlights: Highlight[], onRemoveHighlight: (id: string) => void, getClickToRemoveText: () => string, onClickRef: {
    ...
    const handler = (e: MouseEvent) => {
        ...
        if (selection && selection.toString().trim())
            return;
        const target = (e.target as HTMLElement).closest(`.${USER_HIGHLIGHT_CLASS}`);
...
export const TranscriptWithHighlights = forwardRef<HTMLDivElement | null, {
...
}>(function TranscriptWithHighlights({ transcriptHtml, segmentId, highlights, removeHighlight, answers = {}, correctAnswers, isCorrect = () => null, getCorrectAnswerText = () => null, getBadgeLabel, getBadgeCorrect, }, ref) {
    const { t } = useI18n();
    ...
    useEffect(() => {
        ...
        if (!el || !transcriptHtml)
            return;
        applyHighlightsToTranscript(el, transcriptHtml, segmentHighlights, stableRemoveHighlight, () => t("clickToRemoveHighlight"), onClickRef);
        return () => {
            if (onClickRef.current) {
    ...
    useEffect(() => {
        const el = containerRef.current;
        if (!el || !transcriptHtml || !correctAnswers)
            return;
        applyAnswerFeedbackToTranscript(el, isCorrectRef.current, getBadgeLabelRef.current, getBadgeCorrectRef.current);
    }, [answers, correctAnswers, transcriptHtml]);
    return (<div ref={mergeRefs<HTMLDivElement>(ref, scrollContainerRef)} className="ielts-transcript flex-1 select-text cursor-text overflow-y-auto pr-3">
      <div ref={containerRef} data-segment-id={segmentId} className="highlight-segment transcript-content select-text cursor-text text-sm leading-relaxed text-zinc-800 dark:text-zinc-100" style={{
