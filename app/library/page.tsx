...
import { deleteWordFromStorage, getSavedWordsFromStorage, getSensesFromRow, setSavedStatusById, updateTagsById, } from "@/lib/library-storage";
...
export default function LibraryPage() {
    ...
    const allTags = useMemo(() => {
        ...
        savedWords.forEach((w) => (w.tags ?? []).forEach((tag) => set.add(tag)));
    ...
    return (<div className="mx-auto max-w-4xl space-y-6">
      ...
      <section className="rounded-2xl border border-zinc-200 bg-white p-7 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <h2 className="text-xl font-semibold text-zinc-900 dark:text-zinc-100">
          {t("myWordsCount").replace("{count}", String(savedWords.length))}
        </h2>
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-[1fr_140px_140px]">
          <input placeholder={t("searchInLibrary")} value={search} onChange={(e) => setSearch(e.target.value)} className="h-11 rounded-xl border border-zinc-300 bg-white px-4 text-base text-zinc-900 placeholder:text-zinc-400 outline-none transition focus:border-zinc-900 focus:ring-2 focus:ring-zinc-200 dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-100 dark:placeholder:text-zinc-500 dark:focus:border-zinc-300 dark:focus:ring-zinc-700"/>
